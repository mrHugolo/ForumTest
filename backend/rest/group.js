...
module.exports = function restGroup(app, db, bpJson) {
  ...
  app.get("/rest/groups", bpJson, (req, res) => {
  ...
  app.get("/rest/isJoined/:groupName", bpJson, (req, res) => {
    db.get("SELECT id FROM [group] WHERE name = ?", [req.params.groupName], (err1, row1) => {
      if(err1) throw err1
      db.get("SELECT role FROM [userXgroup] WHERE userId = ? AND groupId = ?", [1, row1.id], (err2, row2) => {
        if (err2) throw err2
  ...
  app.get("/rest/members/:groupName", bpJson, (req, res) => {
    db.get("SELECT id FROM [group] WHERE name = ?", [req.params.groupName], (err1, row1) => {
      if (err1) throw err1
    ...
    db.get("SELECT id FROM [group] WHERE name = ?", [req.body.name], (err1,row1) => {
      ...
      db.get("INSERT INTO [group] (name, description) VALUES (?,?)", [req.body.name, req.body.description], (err2, row2) => {
        ...
        db.get("SELECT id FROM [group] WHERE name = ?", [req.body.name], (err3, row3) => {
          ...
          //TODO: switch userId = 1 to req.session.user.id
        })
      })
    })
  });
  ...
  app.post("/rest/joinGroup", bpJson, (req,res)=> {
    db.get("SELECT id FROM [group] WHERE name = ?", [req.body.name], (err1, row1) => {
      ...
      db.get("SELECT role FROM [userXgroup] WHERE userId = ? AND groupID = ?", [req.body.userId, row1.id], (err2, row2) => {
        if(err2) throw err2
        if(row2 != undefined) {
          res.send({response: false, role: row2?.role})
          return
        }
        db.get("INSERT INTO [userXgroup] (userId, groupId, role) VALUES (?,?,?)", [req.body.userId, row1.id, "Authorized"], (err3, row3) => {
          if (err3) throw err3
          res.send({ response: true })
        })
      })
  ...
  app.delete("/rest/leaveGroup", bpJson, (req, res) => {
    db.get("SELECT id FROM [group] WHERE name = ?", [req.body.name], (err1, row1) => {
      if (err1) throw err1
      if(row1 == undefined) {
        res.send({response: false})
      ...
      db.get("SELECT role FROM [userXgroup] WHERE userId = ? AND groupID = ?", [req.body.userId, row1.id], (err2, row2) => {
        ...
        db.get("DELETE FROM [userXgroup] WHERE userId = ? AND groupId = ?", [req.body.userId, row1.id], (err3, row3) => {
          if (err3) throw err3
          res.send({ response: true })
        })
      })
